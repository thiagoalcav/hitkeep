...
@Component({
  ...
  imports: [
    ...
    Brand, SiteSelector,
    AvatarModule, ButtonModule, MenuModule, DrawerModule, DialogModule, InputTextModule, MessageModule
  ],
  template: `
    <div class="flex h-screen w-full bg-[var(--p-surface-ground)]">
      ...
      <aside class="hidden md:flex w-64 flex-col bg-[var(--p-surface-card)] border-r border-surface-200 dark:border-surface-700 p-4 gap-6" aria-label="Main Sidebar">
        ...
        <app-site-selector
            ...
            (addClicked)="openAddSite()" />

        <nav class="flex-1 flex flex-col gap-1" aria-label="Primary Navigation">
          <div class="text-xs font-semibold text-muted-color uppercase px-2 mb-2" role="presentation">Menu</div>
        ...
        <div class="border-t border-surface-200 dark:border-surface-700 pt-4 flex items-center justify-between px-2">
           ...
           <div class="flex gap-1">
             <button (click)="prefs.toggleTheme()"
                class="p-2 rounded-full hover:bg-surface-200 dark:hover:bg-surface-800 text-muted-color focus:outline-none focus:ring-2 focus:ring-primary-500"
      ...
      <!-- Add Site Dialog (Shared) -->
      <p-dialog header="Add Website" [(visible)]="isAddSiteVisible" [modal]="true" [style]="{ width: '400px' }">
        ...
        <ng-template pTemplate="footer">
            <p-button label="Cancel" (onClick)="isAddSiteVisible.set(false)" styleClass="p-button-text" />
            <p-button label="Add Website" (onClick)="createSite()" [loading]="isCreatingSite()" />
        </ng-template>
      </p-dialog>
    </div>
  `
...
export class MainLayout {
  ...
  protected newSiteDomain = signal('');
  protected isCreatingSite = signal(false);
  ...
  protected createSite() {
    ...
    this.siteService.createSite(domain).subscribe({
      ...
      error: () => {
        this.isCreatingSite.set(false);
        this.createSiteError.set('Failed to create site');
      }
    });
  }
}
