...
export class PermissionService {
  ...
  readonly isInstanceOwner = computed(() => 
    this.permissions()?.instance_role === 'owner'
  );
  
  readonly isInstanceAdmin = computed(() => 
    ['owner', 'admin'].includes(this.permissions()?.instance_role || '')
  );
  
  loadPermissions() {
    return this.http.get<UserPermissions>('/api/user/permissions')
      .pipe(tap(perms => this.permissions.set(perms)));
  }
