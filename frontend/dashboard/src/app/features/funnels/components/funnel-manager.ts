...
import { Funnel, FunnelStep } from '../../../core/models/analytics.types';
                    ...
                    <td>
                        <div class="flex items-center gap-1 flex-wrap">
                            @for (step of funnel.steps; track $index; let last = $last) {
                                <span class="text-xs bg-surface-100 dark:bg-surface-800 px-2 py-1 rounded border border-surface-200 dark:border-surface-700" [title]="step.value">
                                    {{step.type === 'path' ? '/' : ''}}{{step.value}}
                                </span>
                                @if (!last) {
...
export class FunnelManager implements OnChanges {
  ...
  createFunnel() {
    ...
    this.analyticsService.createFunnel(this.siteId, this.newFunnel).subscribe({
      next: () => {
        ...
        this.newFunnel = { 
            name: '', 
            steps: [{ type: 'path', value: '' }, { type: 'path', value: '' }] 
        };
        this.loadFunnels();
  ...
  isValid() {
    return this.newFunnel.name.length > 0 && 
           this.newFunnel.steps.length >= 2 && 
           this.newFunnel.steps.every(s => s.value.length > 0);
  }
