...
@Component({
    ...
    imports: [ReactiveFormsModule, TranslocoPipe, DatePickerModule, DialogModule, ButtonModule, CardModule, TableModule, PageHeader, PageBreadcrumb, RangeToolbar, KpiCard, MetricList, SeriesChart],
...
export class EcommercePage {
    ...
    protected readonly timeRanges = computed(() => {
        this.activeLanguage();
        return this.buildTimeRanges();
    });
    protected readonly selectedRange = linkedSignal<{ label: string; value: string }[], { label: string; value: string }>({
    ...
    protected readonly isCustomRangeVisible = signal(false);
