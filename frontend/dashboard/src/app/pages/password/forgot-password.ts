...
@Component({
    ...
    standalone: true,
    imports: [ReactiveFormsModule, RouterLink, Brand, ButtonModule, InputTextModule, MessageModule],
    template: `
            ...
            <div class="w-full max-w-md">
                <div class="bg-[var(--p-surface-card)] border border-surface-200 dark:border-surface-700 rounded-xl shadow-sm p-6 md:p-8">
                    <div class="mb-8 flex flex-col items-center justify-center gap-2">
                        ...
                        <p class="text-sm text-[var(--p-text-muted-color)] text-center">Enter your email address and we'll send you a link to reset your password.</p>
                    </div>
                    ...
                    @if (successMessage()) {
                        ...
                        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="flex flex-col gap-4">
                            ...
                            <p-button label="Send Reset Link" type="submit" [loading]="isLoading()" class="w-full" [disabled]="form.invalid"> </p-button>

                            <div class="text-center mt-2">
                                <a routerLink="/login" class="text-sm text-[var(--p-text-muted-color)] hover:text-[var(--p-primary-color)] transition-colors"> Back to Sign In </a>
                            </div>
                        </form>
                    }
                </div>
