...
@Component({
    ...
    standalone: true,
    imports: [ReactiveFormsModule, Brand, ButtonModule, PasswordModule, MessageModule],
    styles: [
        `
                ...
                <div class="bg-[var(--p-surface-card)] border border-surface-200 dark:border-surface-700 rounded-xl shadow-sm p-6 md:p-8">
                    ...
                    @if (successMessage()) {
                        ...
                        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="flex flex-col gap-4">
                            ...
                            @if (!token) {
                                <div class="text-center text-red-500 text-sm p-4 border border-red-200 rounded bg-red-50">Invalid link. The token is missing.</div>
                            } @else {
                                <div class="flex flex-col gap-2">
                                    <label for="password" class="text-sm font-medium text-[var(--p-text-color)]">New Password</label>
                                    <p-password id="password" formControlName="password" [toggleMask]="true" [feedback]="true" class="w-full" placeholder="••••••••"></p-password>
                                    <small class="text-xs text-muted-color">Must be at least 8 characters.</small>
                                </div>

                                <p-button label="Reset Password" type="submit" [loading]="isLoading()" class="w-full" [disabled]="form.invalid"> </p-button>
                            }
                        </form>
                    }
                </div>
