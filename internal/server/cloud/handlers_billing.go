...
func (h *handler) handleSignup() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		status := database.CloudSubscriptionStatusFree
...
func (h *handler) handleCreateBillingPortalSession() http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			slog.Error("Failed to load cloud billing account", "error", err, "team_id", activeTenantID)
			http.Error(w, "Internal server error", http.StatusInternalServerError)
			return
		}
		...
		if err != nil {
			slog.Error("Failed to create Stripe billing portal session", "error", err, "team_id", activeTenantID)
